...
def replace_body(code: str, body: str) -> str:
    """Replace the proof body of a translator-emitted theorem.

    The translator always trails its emitted code with a blank line and a
    ``#print axioms`` directive; we anchor on that to identify the proof
    body.  If no anchor is found we fall back to a broader pattern that
    still requires ``:= by`` so that we cannot accidentally rewrite an
    unrelated definition.  If neither anchor matches we return ``None`` so
    the caller can record an honest "no patch site found" diagnostic rather
    than silently keep the original code as if a patch had been applied.
    """
    new_code, n = re.subn(
        r":=\s+by\n.*?\n\n#print axioms",
        f":= {body}\n\n#print axioms",
    ...
    if n > 0:
        return new_code
    # Fallback: rewrite the proof body up to end of file when there is no
    # #print axioms trailer (e.g. for direct-Lean-code callers).
    new_code, n = re.subn(
        r":=\s+by\b[\s\S]*\Z",
        f":= {body}\n",
        code,
