...
function runPythonTool(command: string, payload: unknown) {
...
const AnyPayload = z.record(z.any()).refine((payload) => {
  const target = (payload as Record<string, any>).target;
...
server.registerTool(
  "lean_check",
  { description: "Check a Lean 4 proof attempt and return structured diagnostics.", inputSchema: AnyPayload },
  async (args: Record<string, unknown>) => textResult(runPythonTool("check", args))
);

server.registerTool(
  "shadowproof_validate",
  { description: "Translate/check an NL or direct Lean proof attempt and return a certificate if Lean accepts.", inputSchema: AnyPayload },
  async (args: Record<string, unknown>) => textResult(runPythonTool("validate", args))
);

server.registerTool(
  "shadowproof_check_draft",
  { description: "Statically check an LLM DraftProposal before running Lean.", inputSchema: AnyPayload },
  async (args: Record<string, unknown>) => textResult(runPythonTool("check-draft", args))
);

server.registerTool(
  "shadowproof_validate_draft",
  { description: "Validate an LLM DraftProposal with theorem-lock, security preflight, Lean, and guarded repair.", inputSchema: AnyPayload },
  async (args: Record<string, unknown>) => textResult(runPythonTool("validate-draft", args))
);
